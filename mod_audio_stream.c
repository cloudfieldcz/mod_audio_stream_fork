 ...
 *
 * Fork: adds WRITE_REPLACE support for low-latency audio injection via binary WebSocket frames.
 * Original: https://github.com/amigniter/mod_audio_stream
 */
...
static switch_bool_t capture_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
    ...
    switch (type) {
        ...
        case SWITCH_ABC_TYPE_WRITE_REPLACE:
            return stream_frame_write(bug, user_data);
            break;
                                     ...
                                     char* metadata)
{
    ...
    /* Always add WRITE_REPLACE for binary audio injection */
    flags |= SMBF_WRITE_REPLACE;

    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "adding bug with WRITE_REPLACE.\n");
...
SWITCH_MODULE_LOAD_FUNCTION(mod_audio_stream_load)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_audio_stream API loading (WRITE_REPLACE fork)..\n");
